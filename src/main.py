...
VERSION = "0.1.0"
CONTEXT_SETTINGS = dict(auto_envvar_prefix="KEYCLOAKCLI")
HOME = str(Path.home())
...
class KEYCLOAK_CLI(click.MultiCommand):
...
@click.command(cls=KEYCLOAK_CLI, context_settings=CONTEXT_SETTINGS)
@click.option(
    "--home",
...
@click.option(
    ...
    expose_value=False,
    is_eager=True,
    help="Print the current version of keycloakcli.",
)
@pass_environment
def cli(ctx, verbose, home):
    """
    Keycloak CLI is a tool to automate the creation of realms and clients to easily integrate authentication with your applications.
    """
    ctx.verbose = verbose
    if home is not None:
        ctx.home = home
